...
def master(tag):
	os.system('/bin/samtools depth Pool18_'+tag+'.bam>depth_Pool18_'+tag+'.txt')
	fh=open('depth_Pool18_'+tag+'.txt','r')
	fn=fh.readlines()
	numberlist=[]
	...
	for i in qn:
		if i.startswith('#START'):
			for j in range(0,len(a)):
				ch.write('python ../../alec/ALEC/ALEC/ALEC.py -r ../../reference/chr19.fasta -i Pool18_'+tag+'.sam -x pacbio_ccs -t chr19:'+str(a[j][0])+'-'+str(a[j][1])+'\n')
				ch.write('mv Pool18_'+tag+'.corrected.fasta temp0'+str(j)+'_Pool18_'+tag+'.corrected.fasta\n')
